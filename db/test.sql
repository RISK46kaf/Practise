...
DELIMITER $$
USE `default_schema`$$
CREATE PROCEDURE `Fill_patient_info` (in sexIn bool, in diagIn int, in SNILSIn int, in INNin int, in passportIn VARCHAR(500), in anamIn int,in FIOin text, in insurnumIn text, in OGRNIn text, in InsurIn text, in cardNoIn text, in BenefinNumIn text, in adressCurrIn int, in AdressConstIn int, in birthDateIn date)
BEGIN
