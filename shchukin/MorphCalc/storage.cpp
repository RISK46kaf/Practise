...
bool Storage::confirmSelection()
{
    if (BoundingBoxTool == last_tool_) {
        Rect *_bbox = new Rect;
#ifdef _DEBUG
        qDebug() << rect.getCoordinates();
        qDebug() << rect.getCoordinates().topLeft()/scale_
                 << rect.getCoordinates().bottomRight()/scale_ ;
#endif
        rect.setCoordinates(rect.getCoordinates().normalized());
//        rect.setCoordinates(QRect(rect.getCoordinates().topLeft()/scale_,
//                                  rect.getCoordinates().bottomRight()/scale_));
        *_bbox = rect;
